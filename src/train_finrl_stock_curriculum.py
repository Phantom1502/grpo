...
TOTAL_ITERATIONS = ITERATIONS_PER_SEGMENT * len(segments)  # đủ để đi hết curriculum 1 lượt (chưa tính review)
...
for it in range(TOTAL_ITERATIONS):
    ...
    if (it + 1) % 5 == 0:
        print(
            f"Iter {it+1}/{TOTAL_ITERATIONS} {curriculum.status_str()} | "
            f"reward={stats['avg_reward']:.3f} | success_rate={stats['success_rate']:.2f} | "
            f"entropy={stats['entropy']:.3f}"
        )

    if curriculum.is_finished:
        print("Đã học hết toàn bộ curriculum.")
        break
